...
# conexão com Qdrant Cloud
client = QdrantClient(
    url="h-",
    api_key="-",
)

#Criando a coleção no Qdrant
try:
    ...
    print("Atenção: A coleção já existe")
...
for result in results.points:
    print(f"Item: {result.payload.get('item_name', 'N/A')}") # type: ignore
    print(f"Score: {result.score}")
    print(f"Description: {result.payload['description'][:150]}...") # type: ignore
    print(f"Price: {result.payload.get('price', 'N/A')}") # type: ignore
    print("---")